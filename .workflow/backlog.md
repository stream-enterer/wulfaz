...
- **UI-512** — ESC chain integration test. The `CloseTopmost` keybinding handler has a 6-level priority chain (tooltips → modals → panels → inspector → sidebar → exit) restored from pre-strip code. No existing test exercises this chain end-to-end. Construct a minimal App state with all 6 dismissable layers present, simulate repeated ESC presses, assert each layer dismisses in documented order before reaching `event_loop.exit()`. Verify `save_window_state()` is called on the final exit path.
