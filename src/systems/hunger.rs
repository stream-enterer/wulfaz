...
pub fn run_hunger(world: &mut World, _tick: Tick) {
    let mut changes: Vec<(crate::components::Entity, f32)> = world
        .hungers
        .iter()
...
mod tests {
    ...
    fn test_hunger_clamped_at_max() {
        let mut world = World::new_with_seed(42);
        let e = world.spawn();
        world.hungers.insert(e, Hunger { current: 99.5, max: 100.0 });
        run_hunger(&mut world, Tick(0));
